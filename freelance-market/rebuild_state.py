#!/usr/bin/env python3
"""สร้างไฟล์สถานะกลับมาจากสายข้อมูล — ทางกู้ทางเดียวเมื่อสถานะหายหรือล้า

ตัวเก็บจบด้วยรหัสผิดพลาดเมื่อหาสถานะไม่เจอทั้งที่ที่เก็บมีไฟล์รายวันอยู่ (ดู `store.load_state`)
สคริปต์นี้คือสิ่งที่คนที่ถูกเรียกมาดูต้องใช้ต่อ ส่วนสถานะที่มีอยู่แต่ล้ากว่าบันทึกการรัน
ตัวเก็บเรียก `rebuild()` เองตอนเริ่ม ไม่ต้องรอคน มันอ่านสายประกาศ ส่วนต่าง ผลปลายทาง หน้าเว็บ
    ...
    for t, _, stream, row in events:
        ...
        if stream == "listings":
            ...
            if ev == "closed":
                ...
                # n กับ avg ของแถวถามซ้ำไม่ถูกนำกลับเข้าสถานะ — ตัวเก็บเองก็ไม่นำกลับ
                if e["due"]:
                    e["due"].pop(0)
                if not e["due"]:
                    done[pid] = 1
